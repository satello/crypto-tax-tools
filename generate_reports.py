...
from utils.reporting import report_asset_movement, report_asset_FIFO

def FIFO_for_asset(buys, sells, asset):
    asset_short_term, btc_long_term, remaining_portfolio = FIFO(buys, sells, asset)

    report_asset_FIFO(asset_short_term, '%s_short_term_FIFO.csv' % asset)
    report_asset_FIFO(btc_long_term, '%s_long_term_FIFO.csv' % asset)
